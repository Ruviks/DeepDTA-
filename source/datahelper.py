...
class DataSet(object):
    ...
    def parse_data_csv(self, FLAGS, train=True):
        fpath = FLAGS.dataset_path
        print("Read %s start" % fpath)
        if train:
            obj = pd.read_csv(fpath + "BA.csv")
        else:
            obj = pd.read_csv(fpath + "BATest.csv")
        XD = []
        XT = []
        for d in obj["SMILES"]:
            XD.append(label_smiles(d, self.SMILEN, self.charsmiset))

        for t in obj["receptor"]:
            XT.append(label_sequence(t, self.SEQLEN, self.charseqset))

        return XD, XT, obj["Energy"]
    ...
    def parse_data(self, FLAGS, with_label=True):
        ...
        # READS BINARY
        Y = pickle.load(open(fpath + "Y", "rb"), encoding='latin1')  ### TODO: read from raw FUCK
        if FLAGS.is_log:
